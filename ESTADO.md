...
- [x] 🎬 **Cartão em Vídeo** (`cartao-video.html`, página de teste separada): o Cartão da Unha premium ganha vida num filme de ~5s — foto com zoom de cinema, moldura dourada se desenhando, nome/serviço/data entrando, moedas de selo acendendo uma a uma (com pop e anel de brilho), brilho varrendo o cartão, "faltam X" com dourado passando, QR surgindo e confete dourado no final. Grava no próprio celular (`canvas.captureStream` + `MediaRecorder`, sai **MP4 H.264 ~1-3MB** pronto pro status) e compartilha via Web Share; fallback download. Testado com Playwright: frames-chave conferidos + **MP4 gravado e reproduzido de volta (5,74s, 1080×1350, decodifica OK)**, 0 erros. Ideia: vídeo tem muito mais alcance no status/reels que foto → turbina o motor viral
